...
- **主界面**：今日进度环、文档搜索、全部 / 进行中 / 已完成 / 有收藏 筛选、排序（最近更新 / 名称 / 进度）、置顶、
  环形进度卡片、展开后的卡片操作动画。
- **翻译界面**：上下分栏（分割线可调可锁）、逐行 / 逐句切换、撤销重做、查找替换、
  跳转列表（可筛选未完成 / 收藏）、朗读、收藏、左右滑动切换上下句、Ctrl+Enter 下一句。
...
### 局域网 Web 终端
- 设置中开启后，手机启动前台服务监听端口；局域网设备浏览器打开
  `http://<手机IP>:<端口>/terminal` 即可使用 xterm.js 终端（命令运行在手机本地 shell，非 root）。
- 支持**访问令牌**（填写后需带 `?token=xxx`）、自定义端口、应用启动后自动开启。
- xterm.js 与样式已打包进 `assets/web_terminal`，完全离线可用；页面提供状态指示、清屏、
  字号调整与手动重连。
